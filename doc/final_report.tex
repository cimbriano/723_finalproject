...
\section{ Approach }
    ...
    We used a bigram language model over phonemes since our assumption was that phonetic transcriptions would be input to our distinguisher, and pragmatically, we could use some of the Project 1 \cite{hal} infrastructure and leverage our experience with the Carmel Finite State Machine \cite{carmel}.
...
We used the Speech Accent Archive as our test data set for the distinguisher \cite{saa}.
    ...
    An overall score was increased by one if the word had a higher probability ffor the American source than the British source, and decreased by one if the word had a higher probabilty for the British source.
    ...
    We then found the percentage of paragraphs that the distinguisher outputted the correct source for and used this as our final measure for this evaluation.
...
\section{Complications}
	...
	\subsection{Phonetic Granularity}
        ...
        Similarly, we changed R-colored vowels, which only appeared in the Speech Accent Archive to instead be the vowel followed by an r.
...
\section{ Results }
	...
	Our distinguisher consistently gets about 10\% accuracy on this dataset, which is exactly the number of British examples.
	In other words, the distinguisher is very biased towards British and thus attempts to label all examples as British.
	The low accuracy is an artifact of the skewed distribution of British versus American samples.
	Thus given more test data sets with a roughly even distribution of British versus American, we'd expect to do no better or worse than chance.
	...
	This is not to suggest that the OED is biased towards their mother tongue, but it may be the case that British speakers produce phonetics closer to the "accepted standard" the OED subscribes to.
	 ...
	 If these two effects were present, British pronunciations would be closer to the language model produces and American pronunciations, farther.
...
%	\bibitem{sads} Elaine Shi, Charalampos Papamanthou \emph{Streaming Authenticated Data Structures}, Date Unknown
    ...
    \bibitem{hal} Daumé III, H. \textit{CS 723 Project 1: Morphological Disambiguation} [Computer program]. Retrieved December 15, 2012 from http://www.umiacs.umd.edu/~hal/courses/2012F_CL1/p1/
\end{thebibliography}
